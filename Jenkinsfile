...
def targetMap = [ master: 'dev', stable: 'uat', test: 'va'] // Environment mapping: branch -> env
    ...
    if (target) {
      ...
      container('nodejs') {
        ...
        stage('Nexus') {
          ...
          def tarball = (target == 'uat' || target == 'va') ? "${app}-${version}.tar.gz" : "${app}.tar.gz"
      ...
      if (target == 'dev' || target == 'uat' || target == 'va') {
        stage ('Kubernetes') {
